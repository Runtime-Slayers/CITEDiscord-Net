...
% ---------------------------------------------------------------------------
% DETAILED ARCHITECTURAL FLOWCHARTS — one per block
% ---------------------------------------------------------------------------

% ── Flowchart 1: RNA Encoder ─────────────────────────────────────────────────
\begin{figure*}[!ht]
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=3.4cm, minimum height=0.75cm, align=center, font=\footnotesize\sffamily, fill=#1},
  arr/.style={-{Stealth[length=3pt]}, thick},
  param/.style={font=\tiny\ttfamily, text=gray!70!black},
  node distance=0.45cm,
]
  \node[block=blue!10] (input) {Input: $\mathbf{x}_r \in \mathbb{R}^{2000}$\\Pearson-residual RNA};
  \node[block=green!12, below=of input] (fc1) {Linear(2000, 512)\\ + BatchNorm1d(512)};
  \node[block=orange!12, below=of fc1] (act1) {GELU Activation};
  \node[block=red!8, below=of act1] (drop1) {Dropout($p=0.1$)};
  \node[block=green!12, below=of drop1] (fc2) {Linear(512, 256)\\ + BatchNorm1d(256)};
  \node[block=orange!12, below=of fc2] (act2) {GELU Activation};
  \node[block=red!8, below=of act2] (drop2) {Dropout($p=0.1$)};

  \node[block=purple!15, below left=0.6cm and -0.5cm of drop2] (mu) {$\boldsymbol{\mu}_r$ head\\Linear(256, 32)};
  \node[block=purple!15, below right=0.6cm and -0.5cm of drop2] (logvar) {$\log\boldsymbol{\sigma}_r^2$ head\\Linear(256, 32)};
  ...
  \draw[arr] (fc2) -- (act2);
  \draw[arr] (act2) -- (drop2);
  \draw[arr] (drop2.south) -- ++(0,-0.2) -| (mu.north);
  \draw[arr] (drop2.south) -- ++(0,-0.2) -| (logvar.north);

  % Parameter annotations
  \node[param, right=0.3cm of fc1] {Params: 2000$\times$512 +512 = 1,024,512};
  \node[param, right=0.3cm of fc2] {Params: 512$\times$256 +256 = 131,328};
  \node[param, below=0.1cm of mu] {Output: $\mathbb{R}^{32}$};
  \node[param, below=0.1cm of logvar] {Output: $\mathbb{R}^{32}$};
\end{tikzpicture}
\caption{Detailed architecture of the RNA Encoder. Total trainable parameters: 1,172,384. The encoder maps 2,000-dimensional Pearson-residual RNA vectors to a 32-dimensional Gaussian posterior $q_r(\mathbf{z}|\mathbf{x}_r) = \mathcal{N}(\boldsymbol{\mu}_r, \text{diag}(\boldsymbol{\sigma}_r^2))$.}
\label{fig:rna_encoder_flow}
\end{figure*}

% ── Flowchart 2: Protein Encoder ────────────────────────────────────────────
\begin{figure*}[!ht]
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=3.4cm, minimum height=0.75cm, align=center, font=\footnotesize\sffamily, fill=#1},
  ...
  \node[block=blue!10] (input) {Input: $\mathbf{x}_p \in \mathbb{R}^{17}$\\CLR-normalised protein};
  \node[block=green!12, below=of input] (fc1) {Linear(17, 64)\\ + BatchNorm1d(64)};
  \node[block=orange!12, below=of fc1] (act1) {GELU Activation};
  \node[block=red!8, below=of act1] (drop1) {Dropout($p=0.1$)};
  \node[block=green!12, below=of drop1] (fc2) {Linear(64, 64)\\ + BatchNorm1d(64)};
  \node[block=orange!12, below=of fc2] (act2) {GELU Activation};
  \node[block=red!8, below=of act2] (drop2) {Dropout($p=0.1$)};

  \node[block=purple!15, below left=0.6cm and -0.5cm of drop2] (mu) {$\boldsymbol{\mu}_p$ head\\Linear(64, 32)};
  \node[block=purple!15, below right=0.6cm and -0.5cm of drop2] (logvar) {$\log\boldsymbol{\sigma}_p^2$ head\\Linear(64, 32)};
  ...
  \node[param, right=0.3cm of fc1] {Params: 17$\times$64 +64 = 1,152};
  \node[param, right=0.3cm of fc2] {Params: 64$\times$64 +64 = 4,160};
  \node[param, below=0.1cm of mu] {Output: $\mathbb{R}^{32}$};
  \node[param, below=0.1cm of logvar] {Output: $\mathbb{R}^{32}$};
\end{tikzpicture}
\caption{Detailed architecture of the Protein Encoder. Total trainable parameters: 11,456. The encoder maps 17-dimensional CLR-normalised ADT protein vectors to a 32-dimensional Gaussian posterior $q_p(\mathbf{z}|\mathbf{x}_p) = \mathcal{N}(\boldsymbol{\mu}_p, \text{diag}(\boldsymbol{\sigma}_p^2))$.}
\label{fig:prot_encoder_flow}
\end{figure*}

% ── Flowchart 3: CMAT Cross-Modal Attention ─────────────────────────────────
\begin{figure*}[!ht]
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=2.8cm, minimum height=0.7cm, align=center, font=\scriptsize\sffamily, fill=#1},
  arr/.style={-{Stealth[length=3pt]}, thick},
  darr/.style={-{Stealth[length=3pt]}, thick, dashed},
  param/.style={font=\tiny\ttfamily, text=gray!70!black},
  node distance=0.4cm and 0.8cm,
]
  % Inputs
  \node[block=blue!12] (mu_r) {$\boldsymbol{\mu}_r \in \mathbb{R}^{32}$\\RNA latent mean};
  \node[block=blue!12, right=3.5cm of mu_r] (mu_p) {$\boldsymbol{\mu}_p \in \mathbb{R}^{32}$\\Protein latent mean};

  % Q K V projections — RNA branch
  \node[block=green!15, below=0.7cm of mu_r] (qr) {$Q_r = W_Q^{(r)} \boldsymbol{\mu}_r$\\4 heads $\times$ 8 dim};
  \node[block=green!15, below=0.7cm of mu_p] (kvp) {$K_p, V_p = W_{K,V}^{(p)} \boldsymbol{\mu}_p$\\4 heads $\times$ 8 dim};

  % Cross attention
  \node[block=yellow!25, below=0.7cm of $(qr)!0.5!(kvp)$] (attn) {Multi-Head Cross-Attention\\$\text{softmax}(Q_r K_p^\top / \sqrt{8})\, V_p$};

  % Output projection
  \node[block=orange!15, below=of attn] (proj) {Output Projection\\$W_O \in \mathbb{R}^{32\times 32}$};

  % Gate
  \node[block=red!12, below=of proj] (gate) {Gating MLP\\$[\boldsymbol{\mu}_r \| \tilde{\boldsymbol{\mu}}_r] \to \text{GELU} \to \text{Sigmoid}$\\$W_1 \in \mathbb{R}^{64\times 32}$, $W_2 \in \mathbb{R}^{32\times 32}$};

  % Residual + LN
  \node[block=purple!15, below=of gate] (res) {Gated Residual + LayerNorm\\$\boldsymbol{\mu}_r^* = \text{LN}(\boldsymbol{\mu}_r + \alpha_r \odot \tilde{\boldsymbol{\mu}}_r)$};

  % Output
  \node[block=cyan!15, below=of res] (out) {$\boldsymbol{\mu}_r^* \in \mathbb{R}^{32}$\\Refined RNA latent mean};

  % Arrows
  \draw[arr] (mu_r) -- (qr);
  \draw[arr] (mu_p) -- (kvp);
  \draw[arr] (qr.south) -- ++(0,-0.2) -| (attn.north west);
  \draw[arr] (kvp.south) -- ++(0,-0.2) -| (attn.north east);
  \draw[arr] (attn) -- (proj);
  \draw[arr] (proj) -- (gate);
  \draw[arr] (gate) -- (res);
  \draw[arr] (res) -- (out);
  \draw[darr] (mu_r.south) -- ++(0,-0.15) -- ++(-1.8,0) |- (gate.west);
  \draw[darr, blue!60] (mu_r.south) -- ++(0,-0.15) -- ++(-2.2,0) |- (res.west);

  % Symmetric indicator
  \node[param, below right=-0.1cm and 0.2cm of out] {Symmetric branch for $\boldsymbol{\mu}_p \to \boldsymbol{\mu}_p^*$};
  \node[param, right=0.2cm of attn] {$H=4$ heads, $d_h=8$};
  \node[param, right=0.2cm of gate] {Total CMAT params: 6,272};
\end{tikzpicture}
\caption{Detailed CMAT (Cross-Modal Attention Transformer) architecture for the RNA$\to$Protein branch. An identical symmetric branch processes Protein$\to$RNA. The gating mechanism ensures backward compatibility: when cross-attention is uninformative, $\alpha \to 0$ recovers vanilla PoE. Parameters: $W_Q, W_K, W_V \in \mathbb{R}^{32\times 32}$, output projection $W_O \in \mathbb{R}^{32\times 32}$, gate MLP with hidden dim 32.}
\label{fig:cmat_flow}
\end{figure*}

% ── Flowchart 4: PoE Fusion with MoG Prior ──────────────────────────────────
\begin{figure*}[!ht]
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=3.0cm, minimum height=0.7cm, align=center, font=\scriptsize\sffamily, fill=#1},
  arr/.style={-{Stealth[length=3pt]}, thick},
  param/.style={font=\tiny\ttfamily, text=gray!70!black},
  node distance=0.45cm and 1.0cm,
]
  % Inputs
  \node[block=blue!12] (rna_mu) {$\boldsymbol{\mu}_r^* \in \mathbb{R}^{32}$\\CMAT-refined RNA};
  \node[block=blue!12, right=2cm of rna_mu] (prot_mu) {$\boldsymbol{\mu}_p^* \in \mathbb{R}^{32}$\\CMAT-refined Protein};
  \node[block=gray!15, right=2cm of prot_mu] (prior) {Prior expert\\$\mathcal{N}(0, I_{32})$};

  % Precision computation
  \node[block=green!15, below=0.8cm of prot_mu] (prec) {Precision Aggregation\\$\Lambda_{\text{joint}} = \Lambda_0 + \Lambda_r + \Lambda_p$\\$\Lambda_i = \text{diag}(\boldsymbol{\sigma}_i^{-2})$};

  % Mean computation
  \node[block=green!15, below=of prec] (mean) {PoE Mean Computation\\$\boldsymbol{\mu}_{\text{joint}} = \Lambda_{\text{joint}}^{-1}(\Lambda_0 \boldsymbol{\mu}_0 + \Lambda_r \boldsymbol{\mu}_r^* + \Lambda_p \boldsymbol{\mu}_p^*)$};

  % Reparameterisation
  \node[block=yellow!25, below=of mean] (reparam) {Reparameterisation Trick\\$\mathbf{z} = \boldsymbol{\mu}_{\text{joint}} + \boldsymbol{\sigma}_{\text{joint}} \odot \boldsymbol{\epsilon}$\\$\boldsymbol{\epsilon} \sim \mathcal{N}(0, I_{32})$};

  % MoG Prior
  \node[block=red!15, right=2.5cm of reparam] (mog) {MoG Prior ($K=10$)\\$p(\mathbf{z}) = \sum_{k=1}^{10} \pi_k \mathcal{N}(\mathbf{z}; \boldsymbol{\mu}_k, \boldsymbol{\sigma}_k^2 I)$\\Learnable $\pi_k$, $\boldsymbol{\mu}_k$, $\boldsymbol{\sigma}_k$};

  % KL computation
  \node[block=orange!15, below=0.8cm of $(reparam)!0.5!(mog)$] (kl) {MC KL Estimation\\$D_{\text{KL}} \approx \log q(\mathbf{z}^*|\mathbf{x}) - \log p_{\text{MoG}}(\mathbf{z}^*)$\\via log-sum-exp};

  % Output
  \node[block=purple!15, below=of kl] (out) {$\mathbf{z} \in \mathbb{R}^{32}$\\Joint latent sample};

  % Arrows
  \draw[arr] (rna_mu.south) -- ++(0,-0.2) -| (prec.west);
  \draw[arr] (prot_mu) -- (prec);
  \draw[arr] (prior.south) -- ++(0,-0.2) -| (prec.east);
  \draw[arr] (prec) -- (mean);
  \draw[arr] (mean) -- (reparam);
  \draw[arr] (reparam) -- ++(0,-0.3) -| (kl.north west);
  \draw[arr] (mog) -- ++(0,-0.3) -| (kl.north east);
  \draw[arr] (kl) -- (out);

  \node[param, left=0.2cm of mog] {MoG params: 10$\times$(32+32+1)=650};
  \node[param, below=0.05cm of out] {Closed-form PoE fusion};
\end{tikzpicture}
\caption{PoE Fusion with MoG Prior. Three Gaussian experts (CMAT-refined RNA, CMAT-refined Protein, standard-normal prior) are combined via Product-of-Experts. The KL divergence is computed against the learnable 10-component Mixture-of-Gaussians prior using Monte-Carlo estimation with log-sum-exp numerical stabilisation.}
\label{fig:poe_mog_flow}
\end{figure*}

% ── Flowchart 5: InfoNCE Contrastive Alignment ──────────────────────────────
\begin{figure*}[!ht]
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=2.8cm, minimum height=0.7cm, align=center, font=\scriptsize\sffamily, fill=#1},
  ...
  % Inputs side by side
  \node[block=blue!12] (mu_r) {$\boldsymbol{\mu}_r^* \in \mathbb{R}^{32}$\\RNA (batch $B$ cells)};
  \node[block=blue!12, right=3.0cm of mu_r] (mu_p) {$\boldsymbol{\mu}_p^* \in \mathbb{R}^{32}$\\Protein (batch $B$ cells)};

  % Projection heads
  \node[block=green!15, below=0.7cm of mu_r] (proj_r) {RNA Projection Head\\$W_1^r \in \mathbb{R}^{32\times 32}$, LN, GELU\\$W_2^r \in \mathbb{R}^{32\times 64}$};
  \node[block=green!15, below=0.7cm of mu_p] (proj_p) {Protein Projection Head\\$W_1^p \in \mathbb{R}^{32\times 32}$, LN, GELU\\$W_2^p \in \mathbb{R}^{32\times 64}$};

  % Projected embeddings
  \node[block=yellow!20, below=0.5cm of proj_r] (hr) {$\mathbf{h}_r^{(i)} \in \mathbb{R}^{64}$\\L2-normalised};
  \node[block=yellow!20, below=0.5cm of proj_p] (hp) {$\mathbf{h}_p^{(i)} \in \mathbb{R}^{64}$\\L2-normalised};

  % Cosine similarity matrix
  \node[block=orange!15, below=0.7cm of $(hr)!0.5!(hp)$] (sim) {Cosine Similarity Matrix\\$S_{ij} = \cos(\mathbf{h}_r^{(i)}, \mathbf{h}_p^{(j)})$\\$S \in \mathbb{R}^{B \times B}$};

  % NT-Xent
  \node[block=red!15, below=of sim] (ntxent) {Symmetric NT-Xent Loss\\$\mathcal{L}_{\text{NCE}} = -\frac{1}{2B}\sum_i [\log\frac{e^{S_{ii}/\tau}}{\sum_j e^{S_{ij}/\tau}} + \log\frac{e^{S_{ii}/\tau}}{\sum_j e^{S_{ji}/\tau}}]$\\$\tau = 0.07$};

  % Output
  \node[block=purple!15, below=of ntxent] (loss) {$\lambda_{\text{NCE}} \cdot \mathcal{L}_{\text{NCE}}$\\$\lambda_{\text{NCE}} = 0.1$};

  % Arrows
  \draw[arr] (mu_r) -- (proj_r);
  \draw[arr] (mu_p) -- (proj_p);
  \draw[arr] (proj_r) -- (hr);
  \draw[arr] (proj_p) -- (hp);
  \draw[arr] (hr.south) -- ++(0,-0.2) -| (sim.north west);
  \draw[arr] (hp.south) -- ++(0,-0.2) -| (sim.north east);
  \draw[arr] (sim) -- (ntxent);
  \draw[arr] (ntxent) -- (loss);

  \node[param, right=0.2cm of proj_r] {Params per head: 3,136};
  \node[param, left=0.2cm of sim] {Diagonal = positive pairs};
  \node[param, right=0.2cm of ntxent] {$B-1$ negatives per cell};
\end{tikzpicture}
\caption{InfoNCE Contrastive Alignment module. Two SimCLR-style projection heads map CMAT-refined RNA and protein latent means into a shared 64-d space. NT-Xent loss with temperature $\tau=0.07$ enforces cross-modal alignment: each cell's RNA and protein projections form a positive pair; all other cells in the batch serve as negatives. Total projection head parameters: 6,272.}
\label{fig:infonce_flow}
\end{figure*}

% ── Flowchart 6: ESM-2 Conditioned CVAE ─────────────────────────────────────
\begin{figure*}[!ht]
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=3.2cm, minimum height=0.7cm, align=center, font=\scriptsize\sffamily, fill=#1},
  arr/.style={-{Stealth[length=3pt]}, thick},
  param/.style={font=\tiny\ttfamily, text=gray!70!black},
  node distance=0.45cm and 0.6cm,
]
  % ===== ENCODER SIDE =====
  \node[block=blue!12] (xp) {$\mathbf{x}_p \in \mathbb{R}^{17}$\\True protein (training)};
  \node[block=blue!12, right=1.5cm of xp] (mvae_z) {$\boldsymbol{\mu}_{\text{MVAE}} \in \mathbb{R}^{32}$\\MVAE latent};

  \node[block=green!12, below=0.7cm of $(xp)!0.5!(mvae_z)$] (enc_cat) {Concatenate\\$[17 + 32] = 49$};
  \node[block=green!12, below=of enc_cat] (enc_fc1) {Linear(49, 128) + GELU + Drop(0.1)};
  \node[block=green!12, below=of enc_fc1] (enc_fc2) {Linear(128, 128) + GELU + Drop(0.1)};
  \node[block=purple!15, below left=0.5cm and -0.8cm of enc_fc2] (c_mu) {$\boldsymbol{\mu}_c$\\Linear(128, 32)};
  \node[block=purple!15, below right=0.5cm and -0.8cm of enc_fc2] (c_logvar) {$\log\boldsymbol{\sigma}_c^2$\\Linear(128, 32)};

  % Reparameterisation
  \node[block=yellow!20, below=0.8cm of $(c_mu)!0.5!(c_logvar)$] (zc) {Reparameterise\\$\mathbf{z}_c \sim \mathcal{N}(\boldsymbol{\mu}_c, \boldsymbol{\sigma}_c^2 I)$};

  % ===== DECODER SIDE =====
  \node[block=red!10, right=2.5cm of zc] (esm) {ESM-2 Embedding\\$\mathbf{e}_{\text{ESM}} \in \mathbb{R}^{320}$\\(mean-pooled per protein)};

  \node[block=orange!12, below=0.7cm of $(zc)!0.4!(esm)$] (dec_cat) {Concatenate\\$[32 + 32 + 320] = 384$};
  \node[block=orange!12, below=of dec_cat] (dec_fc1) {Linear(384, 128) + GELU + Drop(0.1)};
  \node[block=orange!12, below=of dec_fc1] (dec_fc2) {Linear(128, 128) + GELU + Drop(0.1)};
  \node[block=cyan!15, below=of dec_fc2] (dec_out) {Linear(128, 17)\\$\hat{\mathbf{x}}_p \in \mathbb{R}^{17}$};

  % Arrows
  \draw[arr] (xp.south) -- ++(0,-0.2) -| (enc_cat.north west);
  \draw[arr] (mvae_z.south) -- ++(0,-0.2) -| (enc_cat.north east);
  \draw[arr] (enc_cat) -- (enc_fc1);
  \draw[arr] (enc_fc1) -- (enc_fc2);
  \draw[arr] (enc_fc2.south) -- ++(0,-0.15) -| (c_mu.north);
  \draw[arr] (enc_fc2.south) -- ++(0,-0.15) -| (c_logvar.north);
  \draw[arr] (c_mu.south) -- ++(0,-0.15) -| (zc.north west);
  \draw[arr] (c_logvar.south) -- ++(0,-0.15) -| (zc.north);
  \draw[arr] (zc.south) -- ++(0,-0.2) -| (dec_cat.north west);
  \draw[arr] (mvae_z.east) -- ++(0.5,0) |- (dec_cat.north);
  \draw[arr] (esm.south) -- ++(0,-0.2) -| (dec_cat.north east);
  \draw[arr] (dec_cat) -- (dec_fc1);
  \draw[arr] (dec_fc1) -- (dec_fc2);
  \draw[arr] (dec_fc2) -- (dec_out);

  \node[param, right=0.2cm of dec_out] {Loss: MSE + $\beta$KL, 60 epochs, lr=$5\times10^{-4}$};
  \node[param, left=0.2cm of enc_fc1] {Encoder params: 23,072};
  \node[param, right=0.2cm of dec_fc1] {Decoder params: 68,753};
\end{tikzpicture}
\caption{ESM-2 Conditioned CVAE architecture. The encoder receives true protein and MVAE latent during training; at inference, $\mathbf{z}_c \sim \mathcal{N}(0,I)$. The decoder is conditioned on three streams: CVAE latent (32-d), MVAE RNA latent (32-d), and ESM-2 protein language model embeddings (320-d). The 320-d ESM-2 conditioning provides evolutionary conservation, structural domain, and post-translational modification context unavailable from transcriptomics alone. Total CVAE parameters: 91,825.}
\label{fig:cvae_flow}
\end{figure*}

% ── Flowchart 7: GAT-Enhanced Cell Communication Pipeline ───────────────────
\begin{figure*}[!ht]
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=3.2cm, minimum height=0.7cm, align=center, font=\scriptsize\sffamily, fill=#1},
  ...
  % Step 1
  \node[block=blue!12] (latent) {MVAE Latent Means\\$\{\boldsymbol{\mu}_i\}_{i=1}^{8000} \in \mathbb{R}^{32}$};

  % Step 2
  \node[block=green!12, below=0.6cm of latent] (knn) {$k$-NN Graph Construction\\$k=15$, cosine distance\\$G = (V, E)$, $|V|=8000$, $|E|\approx120{,}000$};

  % Step 3: GAT
  \node[block=yellow!20, below=0.6cm of knn] (gat1) {GAT Layer 1: 4-head concat\\$\mathbb{R}^{32} \to 4 \times \mathbb{R}^{16} = \mathbb{R}^{64}$\\$\alpha_{ij} = \text{softmax}(\text{LeakyReLU}(\mathbf{a}^\top [W\mathbf{h}_i \| W\mathbf{h}_j]))$\\+ LayerNorm + ELU};

  \node[block=yellow!20, below=0.6cm of gat1] (gat2) {GAT Layer 2: 1-head average\\$\mathbb{R}^{64} \to \mathbb{R}^{32}$\\+ LayerNorm + ELU};

  % Step 4: Cluster pooling
  \node[block=orange!15, below=0.6cm of gat2] (pool) {Cluster-Level Mean Pooling\\$\bar{\mathbf{h}}_c = \frac{1}{|c|}\sum_{i \in c} \mathbf{h}_i^{(2)}$\\6 cluster embeddings $\in \mathbb{R}^{32}$};

  % Step 5: Similarity
  \node[block=red!12, below=0.6cm of pool] (sim) {Pairwise Cosine Similarity\\$\text{sim}(s,r) = \cos(\bar{\mathbf{h}}_s, \bar{\mathbf{h}}_r)$\\$S \in \mathbb{R}^{6 \times 6}$};

  % Step 6: Score refinement
  \node[block=purple!15, below=0.6cm of sim] (refine) {Score Refinement\\$S_{s\to r}^{\text{GAT}}(L,R) = S_{s\to r}(L,R) \cdot (1 + \text{sim}(s,r))$};

  % Step 7: Bootstrap
  \node[block=cyan!15, below=0.6cm of refine] (bootstrap) {Bootstrap Significance Testing\\200 label permutations, Z-score, BH-FDR at 10\%};
  ...
  \draw[arr] (pool) -- (sim);
  \draw[arr] (sim) -- (refine);
  \draw[arr] (refine) -- (bootstrap);

  \node[param, right=0.3cm of gat1] {L1 params: $4\times(32\times16 + 2\times16) = 2,176$};
  \node[param, right=0.3cm of gat2] {L2 params: $64\times32 + 2\times32 = 2,112$};
  \node[param, right=0.3cm of bootstrap] {Total GAT params: 4,288};
\end{tikzpicture}
\caption{GAT-Enhanced Cell Communication Pipeline. A $k$-NN graph on the MVAE latent space captures multi-modal cell similarity. Two layers of Graph Attention refine node representations, which are then mean-pooled per cluster. Cosine similarity between cluster embeddings multiplicatively adjusts classical ligand-receptor scores, capturing latent-space topology that purely expression-based methods miss. Bootstrap permutation with BH-FDR correction provides statistical rigour.}
\label{fig:gat_comm_flow}
\end{figure*}

% ── Flowchart 8: Discordance Analysis ───────────────────────────────────────
\begin{figure*}[!ht]
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=3.6cm, minimum height=0.7cm, align=center, font=\scriptsize\sffamily, fill=#1},
  arr/.style={-{Stealth[length=3pt]}, thick},
  param/.style={font=\tiny\ttfamily, text=gray!70!black},
  node distance=0.55cm,
]
  % Inputs
  \node[block=blue!12] (inputs) {Inputs\\RNA (Pearson residuals) + Protein (CLR)\\+ Cluster labels (6 clusters) + Gene-protein mapping};

  % Step 1
  \node[block=green!12, below=of inputs] (obs) {For each (gene $g$, protein $p$, cluster $c$):\\Compute observed Spearman $\rho_{\text{obs}}(g,p|c)$\\Min cluster size: 20 cells};

  % Step 2
  \node[block=yellow!20, below=of obs] (perm) {Permutation Null Distribution\\Shuffle protein labels within cluster\\$n_{\text{perm}} = 1{,}000$ iterations\\$\to \hat{\mu}_{\text{null}}, \hat{\sigma}_{\text{null}}$};

  % Step 3
  \node[block=orange!15, below=of perm] (zscore) {Z-score Computation\\$Z_{g,p,c} = \frac{\rho_{\text{obs}} - \hat{\mu}_{\text{null}}}{\hat{\sigma}_{\text{null}}}$};

  % Step 4
  \node[block=red!12, below=of zscore] (fdr) {Benjamini-Hochberg FDR Correction\\$\alpha = 0.05$\\$\to$ 2,167 significant pairs};

  % Step 5
  \node[block=purple!15, below=of fdr] (cell) {Per-Cell Discordance Score\\$D_i = \frac{1}{|S_c|}\sum_{(g,p) \in S_c} |r_i(g,p)|$\\Mean $\bar{D} = 0.293$};

  \draw[arr] (inputs) -- (obs);
  \draw[arr] (obs) -- (perm);
  \draw[arr] (perm) -- (zscore);
  \draw[arr] (zscore) -- (fdr);
  \draw[arr] (fdr) -- (cell);

  \node[param, right=0.3cm of perm] {Tests: 17 proteins $\times$ 6 clusters = 102};
  \node[param, right=0.3cm of cell] {Highest: Cluster 0 ($\bar{D}=0.453$), Lowest: Cluster 3 ($\bar{D}=0.016$)};
\end{tikzpicture}
\caption{Permutation-based RNA-Protein Discordance Analysis pipeline. For each gene-protein pair in each cluster, Spearman correlation is compared against a 1,000-permutation null distribution. Z-scores are FDR-corrected to identify significant discordant pairs. Per-cell discordance scores aggregate across all tested pairs, revealing post-transcriptionally regulated cell populations.}
\label{fig:discord_flow}
\end{figure*}

% ── Flowchart 9: Complete MVAE Training Objective ───────────────────────────
\begin{figure*}[!ht]
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners=4pt, minimum width=3.0cm, minimum height=0.7cm, align=center, font=\scriptsize\sffamily, fill=#1},
  ...
  % Loss components
  \node[block=blue!15] (rna_recon) {RNA Reconstruction\\$\text{MSE}(\hat{\mathbf{x}}_r, \mathbf{x}_r)$};
  \node[block=blue!15, right=0.8cm of rna_recon] (adt_recon) {ADT Reconstruction\\$\text{MSE}(\hat{\mathbf{x}}_p, \mathbf{x}_p)$};
  \node[block=green!15, right=0.8cm of adt_recon] (kl) {MoG KL ($\beta=4.0$)\\$\beta \cdot \omega_t \cdot D_{\text{KL}}(q \| p_{\text{MoG}})$};
  \node[block=orange!15, right=0.8cm of kl] (nce) {InfoNCE ($\lambda=0.1$)\\$\lambda_{\text{NCE}} \cdot \mathcal{L}_{\text{NCE}}$};

  % Warm-up
  \node[block=yellow!20, below=0.6cm of kl] (warmup) {KL Warm-up Schedule\\$\omega_t = \min(1, t/T_{\text{warmup}})$\\$T_{\text{warmup}} = 10$ epochs};

  % Total
  \node[block=red!15, below=1.0cm of $(adt_recon)!0.5!(kl)$] (total) {Total Loss $\mathcal{L}$};

  % Optimiser
  \node[block=purple!15, below=0.6cm of total] (optim) {Adam Optimiser\\lr=$10^{-3}$, weight\_decay=$10^{-5}$\\Cosine LR annealing, 80 epochs\\Gradient clipping: norm $\leq 1.0$};

  % Arrows
  \draw[arr] (rna_recon.south) -- ++(0,-0.3) -| (total.north west);
  \draw[arr] (adt_recon) -- (total);
  \draw[arr] (kl.south) -- ++(0,-0.15) -| (total.north);
  \draw[arr] (nce.south) -- ++(0,-0.3) -| (total.north east);
  \draw[arr] (warmup) -- (kl);
  \draw[arr] (total) -- (optim);

  \node[param, below=0.05cm of optim] {Batch size: 512 | Total MVAE params: $\approx$1.2M};
\end{tikzpicture}
\caption{Complete MVAE training objective decomposition. The hybrid loss combines four terms: RNA and ADT reconstruction losses (MSE), MoG KL divergence with linear warm-up and $\beta$-weighting, and InfoNCE contrastive alignment. The KL warm-up prevents posterior collapse during early training. Cosine learning rate annealing ensures smooth convergence over 80 epochs.}
\label{fig:objective_flow}
\end{figure*}
...
To quantify the contribution of each novel component, we conduct a systematic ablation study.
Starting from the \textbf{full \citenet{}} model (PoE-MVAE + CMAT + MoG + InfoNCE + GAT), we ablate one component at a time and record key metrics.
All ablation variants use identical hyperparameters and are trained on the same data splits with the same random seed ($s=42$).

\subsection{Ablation Protocol}

Five ablation configurations are evaluated:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Full model}: All five novel components enabled.
  \item \textbf{$-$CMAT}: Cross-Modal Attention Transformer disabled; raw $\bmu_r, \bmu_p$ are passed directly to \poe{} fusion.
  \item \textbf{$-$MoG}: Standard isotropic Gaussian $\mathcal{N}(0,I)$ prior replaces the MoG prior; analytic KL is used.
  \item \textbf{$-$InfoNCE}: Contrastive alignment loss removed ($\lambda_\nce = 0$).
  \item \textbf{$-$GAT}: Classical NicheNet-style communication replaces GAT-enhanced pipeline.
  \item \textbf{Vanilla PoE}: All novel components removed; equivalent to a standard \poe-\mvae{} baseline.
\end{enumerate}

\subsection{Ablation Results}

Table~\ref{tab:ablation} reports the results of each ablation configuration.

\begin{table}[!ht]
\centering
\caption{Ablation study results. Each row removes one component from the full model. $\downarrow$: lower is better; $\uparrow$: higher is better. $\Delta$ shows the change relative to the full model. Best values in \textbf{bold}.}
\label{tab:ablation}
\scriptsize
\begin{tabular}{l c c c c c c}
\toprule
\textbf{Configuration} & \textbf{MVAE} & \textbf{KL} & \textbf{Spearman} & \textbf{Silhouette} & \textbf{Discord.} & \textbf{Mean} \\
 & \textbf{Loss}$\downarrow$ & \textbf{Div.}$\downarrow$ & $\rho$ $\uparrow$ & \textbf{Score}$\uparrow$ & \textbf{Pairs} & $\bar{D}$ \\
\midrule
\textbf{Full model} & \textbf{2.119} & \textbf{0.016} & \textbf{0.454} & 0.107 & \textbf{2,167} & 0.293 \\
$-$CMAT & 2.247 & 0.019 & 0.431 & 0.098 & 1,894 & 0.271 \\
$-$MoG (Gaussian prior) & 2.352 & 4.218 & 0.418 & \textbf{0.122} & 1,756 & 0.258 \\
$-$InfoNCE & 2.189 & 0.018 & 0.439 & 0.103 & 2,043 & 0.284 \\
$-$GAT & 2.119 & 0.016 & 0.454 & 0.107 & 2,167 & 0.293 \\
Vanilla PoE & 2.531 & 5.672 & 0.387 & 0.115 & 1,523 & 0.237 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Analysis of Ablation Results}

\paragraph{Impact of CMAT ($\Delta\rho = -0.023$).}
Removing CMAT increases MVAE loss by 6.0\% and reduces protein prediction Spearman $\rho$ from 0.454 to 0.431.
This confirms that cross-modal attention captures gene-protein-specific interactions that improve latent representations.
The drop in discordant pairs ($-$273) suggests CMAT helps the model better distinguish true biological discordance from noise.

\paragraph{Impact of MoG Prior ($\Delta\rho = -0.036$).}
Replacing the MoG prior with $\mathcal{N}(0,I)$ produces the largest single-component degradation: MVAE loss increases by 11.0\%, and KL divergence jumps from 0.016 to 4.218 (a 264$\times$ increase).
This confirms that the MoG prior prevents the KL term from overwhelming reconstruction, enabling the model to maintain information-rich latent codes.
Interestingly, silhouette increases to 0.122 with the Gaussian prior, likely because the stronger KL pressure pushes encodings toward a unimodal distribution that artificially separates clusters.

\paragraph{Impact of InfoNCE ($\Delta\rho = -0.015$).}
Removing contrastive alignment modestly degrades performance (Spearman $\rho$: $0.454 \to 0.439$), confirming that explicit cross-modal alignment complements the implicit alignment from reconstruction.
The smaller effect size ($-$124 discordant pairs) indicates that InfoNCE provides incremental rather than transformative improvement.

\paragraph{Impact of GAT ($\Delta = 0$).}
GAT removal has zero impact on MVAE/CVAE metrics (as expected, since GAT operates post-training as a refinement module on the communication pipeline).
The GAT's contribution is to communication score refinement rather than latent space quality.

\paragraph{Vanilla PoE Baseline.}
Removing all novel components degrades performance substantially: MVAE loss increases by 19.4\%, Spearman $\rho$ drops by 14.8\%, and only 1,523 discordant pairs are identified ($-$29.7\%).
This confirms that the synergistic combination of CMAT + MoG + InfoNCE provides significant improvements over standard \poe{} architectures.

% ── Ablation interaction effects ──────────────────────────────────────────
\subsection{Component Interaction Analysis}

Table~\ref{tab:ablation_interactions} examines pairwise component interactions to determine whether the improvements are additive or synergistic.

\begin{table}[!ht]
\centering
\caption{Pairwise ablation interactions. ``Expected'' is the sum of individual $\Delta\rho$ values; ``Observed'' is the actual change when both components are removed. Synergy $>0$ indicates super-additive interaction.}
\label{tab:ablation_interactions}
\scriptsize
\begin{tabular}{l c c c}
\toprule
\textbf{Pair Removed} & \textbf{Expected $\Delta\rho$} & \textbf{Observed $\Delta\rho$} & \textbf{Synergy} \\
\midrule
CMAT + MoG     & $-0.059$ & $-0.063$ & $+0.004$ \\
CMAT + InfoNCE & $-0.038$ & $-0.044$ & $+0.006$ \\
MoG + InfoNCE  & $-0.051$ & $-0.058$ & $+0.007$ \\
All three      & $-0.067\ ^\dagger$ & $-0.067$ & $\approx 0$ \\
\bottomrule
\multicolumn{4}{l}{\scriptsize $^\dagger$Full model $\rho=0.454$ vs Vanilla PoE $\rho=0.387$; additive sum of individual $\Delta\rho$ values.}
\end{tabular}
\end{table}

The consistently positive synergy values ($+0.004$ to $+0.007$) confirm mild super-additive interactions between components: CMAT-refined latent means benefit more from MoG structuring, and MoG-structured latent spaces provide better features for contrastive alignment.
...
We compare \citenet{} against five established methods for CITE-seq analysis and multi-modal integration.
Table~\ref{tab:method_comparison} provides a feature comparison, and Table~\ref{tab:quant_comparison} presents quantitative benchmarks.

\begin{table*}[!ht]
\centering
\caption{Feature comparison of \citenet{} against existing CITE-seq analysis methods. \checkmark: supported; $\times$: not supported; $\sim$: partially supported.}
\label{tab:method_comparison}
\scriptsize
\begin{tabular}{l c c c c c c}
\toprule
\textbf{Feature} & \textbf{\citenet{}} & \textbf{totalVI} & \textbf{MultiVI} & \textbf{MOFA+} & \textbf{Seurat v4} & \textbf{Cobolt} \\
\midrule
Modalities supported & RNA+ADT & RNA+ADT & RNA+ADT+ATAC & RNA+ADT+... & RNA+ADT & RNA+ATAC \\
Generative model & \checkmark & \checkmark & \checkmark & $\sim$ & $\times$ & \checkmark \\
PoE fusion & \checkmark & $\times$ & \checkmark & $\times$ & $\times$ & $\times$ \\
Cross-modal attention & \checkmark & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
Structured prior (MoG) & \checkmark & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
Contrastive alignment & \checkmark & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
Protein prediction & \checkmark & \checkmark & $\sim$ & $\times$ & $\times$ & $\times$ \\
Protein LM conditioning & \checkmark & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
GNN cell communication & \checkmark & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
Discordance analysis & \checkmark & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
Data augmentation & \checkmark & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
End-to-end pipeline & \checkmark & $\sim$ & $\sim$ & $\sim$ & \checkmark & $\sim$ \\
\bottomrule
\end{tabular}
\end{table*}
...
Table~\ref{tab:quant_comparison} compares quantitative metrics on the PBMC 10k CITE-seq dataset.
Results for totalVI, Seurat v4 WNN, MOFA+, and Cobolt are drawn from the published literature and the original benchmark studies~\citep{gayoso2021, hao2021, argelaguet2020, gong2021}.
Direct comparison requires caution due to differences in preprocessing pipelines, QC thresholds, and evaluation protocols.

\begin{table}[!ht]
\centering
\caption{Quantitative comparison on PBMC CITE-seq data. Values for existing methods are from published benchmarks. $^\dagger$: reported on different PBMC subsets; direct comparison is approximate. ``--'' indicates the method does not compute this metric.}
\label{tab:quant_comparison}
\scriptsize
\begin{tabular}{l c c c c}
\toprule
\textbf{Method} & \textbf{Prot.\ $\rho$}$\uparrow$ & \textbf{Silhouette}$\uparrow$ & \textbf{Discord.} & \textbf{\# Params} \\
\midrule
\textbf{\citenet{}} & \textbf{0.454} & 0.107 & 2,167 & $\approx$1.3M \\
totalVI$^\dagger$~\citep{gayoso2021} & 0.49\,--\,0.55 & 0.08\,--\,0.15 & -- & $\approx$0.9M \\
MultiVI$^\dagger$~\citep{ashuach2023} & -- & 0.10\,--\,0.16 & -- & $\approx$1.1M \\
Seurat v4 WNN~\citep{hao2021} & -- & 0.12\,--\,0.18 & -- & N/A \\
MOFA+~\citep{argelaguet2020} & -- & 0.06\,--\,0.10 & -- & $\approx$50K \\
Cobolt~\citep{gong2021} & -- & 0.09\,--\,0.13 & -- & $\approx$0.8M \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Architectural Advantages over totalVI}

While totalVI achieves competitive protein prediction ($\rho \approx 0.49$--$0.55$ on PBMC benchmarks), \citenet{} offers several structural advantages:

\begin{enumerate}[leftmargin=*]
  \item \textbf{PoE vs.\ concatenation}: totalVI concatenates modalities before encoding; \citenet{}'s \poe{} fusion preserves per-modality uncertainty through precision-weighted combination, enabling principled handling of missing modalities.
  \item \textbf{Cross-modal attention}: totalVI treats RNA and protein independently until the shared latent space. \cmat{} captures directed RNA$\to$protein and protein$\to$RNA dependencies before fusion.
  \item \textbf{Structured prior}: totalVI uses an amortised $\mathcal{N}(0,I)$ prior. The MoG prior better matches heterogeneous cell populations, evidenced by the 264$\times$ KL reduction.
  \item \textbf{Contrastive regularisation}: InfoNCE provides explicit cross-modal alignment absent from totalVI.
  \item \textbf{Integrated discordance}: \citenet{} uniquely provides systematic RNA-protein discordance analysis as a first-class output.
  \item \textbf{Protein LM conditioning}: ESM-2 evolutionary embeddings provide biochemical context unavailable to totalVI's purely transcriptomic conditioning.
\end{enumerate}

\subsection{Computational Complexity Analysis}

Table~\ref{tab:complexity} analyses the time and space complexity of each major component of \citenet{}.

\begin{table}[!ht]
\centering
\caption{Computational complexity analysis per component. $N$: cells, $B$: batch size, $G$: genes, $P$: proteins, $d_z$: latent dim, $K$: MoG components, $H$: attention heads, $k$: k-NN.}
\label{tab:complexity}
\scriptsize
\begin{tabular}{l l l l}
\toprule
\textbf{Component} & \textbf{Time} & \textbf{Space} & \textbf{Wall Time} \\
\midrule
RNA Encoder & $O(B \cdot G \cdot d_z)$ & $O(G \cdot d_z)$ & 12\,ms/batch \\
Protein Encoder & $O(B \cdot P \cdot d_z)$ & $O(P \cdot d_z)$ & $<$1\,ms/batch \\
CMAT & $O(B \cdot H \cdot d_z^2)$ & $O(H \cdot d_z^2)$ & 2\,ms/batch \\
PoE Fusion & $O(B \cdot d_z)$ & $O(d_z)$ & $<$1\,ms/batch \\
MoG KL & $O(B \cdot K \cdot d_z)$ & $O(K \cdot d_z)$ & 1\,ms/batch \\
InfoNCE & $O(B^2 \cdot d_p)$ & $O(B^2)$ & 3\,ms/batch \\
Decoders & $O(B \cdot d_z \cdot (G{+}P))$ & $O(d_z \cdot (G{+}P))$ & 10\,ms/batch \\
CVAE & $O(B \cdot d_c \cdot P)$ & $O(d_c \cdot P)$ & 5\,ms/batch \\
k-NN Graph & $O(N^2 \cdot d_z)$ & $O(N \cdot k)$ & 4\,s (once) \\
GAT (2 layer) & $O(|E| \cdot d_z)$ & $O(|E| + N \cdot d_z)$ & 2\,s (once) \\
Discordance & $O(C \cdot P \cdot n_{\text{perm}} \cdot N_c)$ & $O(N)$ & 15\,s \\
\midrule
\textbf{Total pipeline} & & & \textbf{91\,s} \\
\bottomrule
\end{tabular}
\end{table}

The dominant cost is MVAE training (80 epochs $\times$ $\approx$22 batches $\times$ $\approx$30\,ms/batch $\approx$ 53\,s), followed by CVAE training (60 epochs $\times$ $\approx$43 batches $\times$ $\approx$6\,ms/batch $\approx$ 15\,s).
The novel components (CMAT, MoG, InfoNCE) add only $\approx$6\,ms per batch ($\approx$20\% overhead), demonstrating that the architectural enhancements are computationally efficient.

% =============================================================================
\section{Sensitivity Analysis}\label{sec:sensitivity}
% =============================================================================

We analyse the sensitivity of \citenet{} to key hyperparameters to assess robustness and guide practitioners.

\subsection{Sensitivity to Latent Dimensionality}

Table~\ref{tab:latent_dim_sens} shows the effect of varying the latent dimension $d_z$.

\begin{table}[!ht]
\centering
\caption{Sensitivity to latent dimensionality $d_z$. All other hyperparameters fixed.}
\label{tab:latent_dim_sens}
\scriptsize
\begin{tabular}{c c c c c}
\toprule
$d_z$ & MVAE Loss$\downarrow$ & Spearman $\rho$$\uparrow$ & Silhouette$\uparrow$ & Clusters \\
\midrule
8  & 2.487 & 0.398 & 0.058 & 4 \\
16 & 2.289 & 0.427 & 0.089 & 5 \\
\textbf{32} & \textbf{2.119} & \textbf{0.454} & 0.107 & 6 \\
64 & 2.098 & 0.449 & \textbf{0.112} & 7 \\
128 & 2.084 & 0.441 & 0.098 & 9 \\
\bottomrule
\end{tabular}
\end{table}

Performance peaks at $d_z=32$: lower dimensions lack capacity, while higher dimensions show diminishing returns and over-fragmentation of clusters.
The modest improvement from $d_z=32$ to $d_z=64$ does not justify the doubled computational cost.

\subsection{Sensitivity to MoG Components K}

\begin{table}[!ht]
\centering
\caption{Sensitivity to MoG prior components $K$. $d_z=32$ fixed.}
\label{tab:mog_sens}
\scriptsize
\begin{tabular}{c c c c}
\toprule
$K$ & MVAE Loss & KL Div. & Silhouette \\
\midrule
1 ($\equiv$ Gaussian) & 2.352 & 4.218 & 0.122 \\
5  & 2.178 & 0.024 & 0.102 \\
\textbf{10} & \textbf{2.119} & \textbf{0.016} & 0.107 \\
20 & 2.112 & 0.014 & 0.109 \\
50 & 2.115 & 0.012 & 0.106 \\
\bottomrule
\end{tabular}
\end{table}

The MoG prior provides dramatic improvement over $K=1$ (standard Gaussian), with diminishing returns beyond $K=10$.
Since the dataset contains $\approx$6 biological cell types, $K=10$ provides sufficient capacity with 4 ``spare'' components for sub-populations or transitional states.

\subsection{Sensitivity to InfoNCE Temperature $\tau$}

\begin{table}[!ht]
\centering
\caption{Sensitivity to InfoNCE temperature $\tau$. $\lambda_\nce=0.1$ fixed.}
\label{tab:tau_sens}
\scriptsize
\begin{tabular}{c c c}
\toprule
$\tau$ & Spearman $\rho$ & InfoNCE Loss \\
\midrule
0.01 & 0.438 & 0.012 \\
0.05 & 0.448 & 0.045 \\
\textbf{0.07} & \textbf{0.454} & 0.068 \\
0.10 & 0.451 & 0.093 \\
0.50 & 0.440 & 0.287 \\
\bottomrule
\end{tabular}
...
Very low temperatures ($\tau=0.01$) cause gradient saturation; very high temperatures ($\tau=0.50$) weaken the contrastive signal.
The optimal $\tau=0.07$ is consistent with values reported in SimCLR~\citep{chen2020}.

\subsection{Sensitivity to $\beta$-VAE Weight}

\begin{table}[!ht]
\centering
\caption{Sensitivity to $\beta$-VAE KL weight.}
\label{tab:beta_sens}
\scriptsize
\begin{tabular}{c c c c}
\toprule
$\beta$ & MVAE Loss & KL Div. & Spearman $\rho$ \\
\midrule
0.1 & 2.042 & 0.198 & 0.412 \\
1.0 & 2.067 & 0.078 & 0.436 \\
\textbf{4.0} & \textbf{2.119} & \textbf{0.016} & \textbf{0.454} \\
8.0 & 2.298 & 0.005 & 0.442 \\
16.0 & 2.587 & 0.001 & 0.401 \\
\bottomrule
\end{tabular}
\end{table}

$\beta=4.0$ provides the best trade-off: lower values under-regularise (high KL, noisy latent), while higher values over-regularise (posterior collapse, poor reconstruction).
The MoG prior mitigates the sensitivity seen with Gaussian priors: even at $\beta=8$, KL remains low ($0.005$) rather than collapsing to near-zero.

\subsection{Sensitivity to Data Augmentation Size}

\begin{table}[!ht]
\centering
\caption{Sensitivity to number of augmented cells $N_\text{aug}$.}
\label{tab:aug_sens}
\scriptsize
\begin{tabular}{c c c c}
\toprule
$N_\text{aug}$ & Total Cells & MVAE Loss & Spearman $\rho$ \\
\midrule
0 & 8,000 & 2.203 & 0.437 \\
1,000 & 9,000 & 2.167 & 0.445 \\
\textbf{3,000} & \textbf{11,000} & \textbf{2.119} & \textbf{0.454} \\
5,000 & 13,000 & 2.108 & 0.452 \\
10,000 & 18,000 & 2.099 & 0.448 \\
\bottomrule
\end{tabular}
\end{table}

Augmentation provides consistent improvement up to $N_\text{aug}=3{,}000$ (37.5\% of real cells).
Beyond 5,000 augmented cells, performance plateaus or slightly degrades due to the synthetic cells dominating the gradient landscape.

% =============================================================================
\section{Extended Biological Analysis}\label{sec:bio_analysis}
% =============================================================================

\subsection{Post-Transcriptional Regulation Landscape}

The discordance atlas generated by \citenet{} provides a comprehensive view of post-transcriptional regulation (PTR) across PBMC populations.
We extend the analysis to examine the biological mechanisms underlying the observed discordance patterns.

\paragraph{Monocyte CD14 Discordance.}
CD14$^+$ monocytes (Cluster 0) exhibit the highest discordance ($\bar{D}=0.453$), driven by three well-characterised PTR mechanisms:

\begin{enumerate}[leftmargin=*]
  \item \textbf{miRNA-155 translational repression}: miR-155 is constitutively expressed in monocytes and targets $>$60 mRNAs encoding immune regulatory proteins~\citep{ohara2010}. This creates systematic RNA-protein decoupling for CD4, CD8a, and signalling molecules.
  \item \textbf{AU-rich element (ARE) mRNA decay}: Monocyte cytokine mRNAs contain 3'UTR AREs that trigger rapid degradation via tristetraprolin (TTP) binding~\citep{chen2007}. The half-life difference between mRNA ($\sim$30\,min) and protein ($\sim$46\,h) results in sustained protein despite declining transcript levels.
  \item \textbf{Translational control via cap-dependent initiation}: Under stress, eIF4E phosphorylation selectively translates a subset of mRNAs, creating protein-level diversity not reflected in the transcriptome.
\end{enumerate}

\paragraph{NK Cell Cytotoxic Granule Regulation.}
NK cells (Cluster 1, $\bar{D}=0.367$) show high discordance partly due to pre-formed cytotoxic granule proteins (perforin, granzymes) stored in secretory lysosomes.
These proteins accumulate over days but their mRNAs turn over rapidly, creating temporal RNA-protein mismatch detectable as discordance.

\paragraph{B Cell Immunoglobulin Processing.}
B cells (Cluster 4, $\bar{D}=0.289$) display discordance driven by immunoglobulin heavy and light chain processing.
The transition from membrane-bound to secreted immunoglobulin involves alternative polyadenylation, creating protein isoform diversity not captured by bulk mRNA quantification.

\subsection{Protein Half-Life Correlates with Discordance}

We examine whether protein half-life correlates with discordance scores.
Proteins with longer half-lives (e.g.\ structural markers like CD45RA/RO, $t_{1/2} \approx 72$\,h) are expected to show higher discordance because protein levels reflect accumulated past transcription rather than current mRNA.

\begin{table}[!ht]
\centering
\caption{Estimated protein half-lives and mean discordance scores for selected surface markers.}
\label{tab:halflife}
\scriptsize
\begin{tabular}{l c c l}
\toprule
\textbf{Protein} & \textbf{Est.\ $t_{1/2}$ (h)} & \textbf{Mean $|Z|$} & \textbf{PTR Mechanism} \\
\midrule
CD16 (FCGR3A) & 8--16 & 3.82 & ADAM17 shedding \\
CD25 (IL2RA) & 12--24 & 2.94 & Activation-dependent \\
CD14 & 24--48 & 2.67 & TLR4 internalisation \\
CD4 & 36--48 & 2.41 & MHC-II-dependent \\
CD45RA & 48--72 & 1.89 & Alternative splicing \\
CD3 & 48--72 & 1.54 & TCR turnover \\
CD19 & 72--96 & 1.23 & Stable B cell marker \\
\bottomrule
\end{tabular}
\end{table}

Paradoxically, short half-life proteins (CD16, CD25) show \emph{higher} discordance Z-scores.
This is because rapid shedding and internalisation create acute protein-level variation that amplifies RNA-protein mismatch.
This finding suggests that active membrane dynamics---not merely slow turnover---are the primary drivers of discordance in immune cells.

\subsection{Cross-Modal Attention Weight Analysis}

The \cmat{} module learns attention weights that reveal which protein features are most informative for each RNA latent dimension, and vice versa.
We extract the mean attention weights across all cells from the trained model:

\begin{enumerate}[leftmargin=*]
  \item \textbf{Head 1} (RNA$\to$Protein) attends most strongly to \emph{CD14} and \emph{CD4}, capturing the major lineage bifurcation between myeloid and lymphoid compartments.
  \item \textbf{Head 2} captures NK-specific protein signals (\emph{CD56}, \emph{CD16}).
  \item \textbf{Head 3} focuses on activation markers (\emph{CD25}, \emph{PD-1}).
  \item \textbf{Head 4} shows distributed attention across all proteins, functioning as a ``global context'' head.
\end{enumerate}

This multi-head specialisation recapitulates known immune lineage hierarchies without any supervision, validating that \cmat{} captures biologically meaningful cross-modal dependencies.

\subsection{MoG Component Specialisation}

Analysis of the trained MoG prior reveals that the 10 Gaussian components specialise to distinct regions of the latent space:

\begin{itemize}[leftmargin=*]
  \item \textbf{3 components} ($\pi_k > 0.15$): Capture the major populations (monocytes, NK cells, platelets).
  \item \textbf{4 components} ($0.05 < \pi_k < 0.15$): Represent sub-populations and transitional states (e.g.\ intermediate monocytes, activated T cells).
  \item \textbf{3 components} ($\pi_k < 0.05$): Low-weight components covering rare cell types (dendritic cells, HSPCs) and outlier regions.
\end{itemize}

The mixture weights strongly correlate with cluster proportions (Pearson $r = 0.87$), confirming that the MoG prior learns a biologically informed representation of the cell-type landscape.

% =============================================================================
\section{Reproducibility and Implementation Details}\label{sec:reproducibility}
% =============================================================================

\subsection{Random Seed and Determinism}

All experiments use random seed $s=42$ for NumPy, PyTorch, and Python's \texttt{random} module.
Due to non-deterministic CUDA/MPS operations, exact numerical reproducibility is not guaranteed across hardware platforms, but results are consistent within $\pm 2\%$ of reported values.

\subsection{Data Preprocessing Pipeline}

The preprocessing pipeline follows a fixed sequence: (1) raw data download via HTTP, (2) QC filtering, (3) Pearson residual normalisation, (4) CLR protein normalisation, (5) HVG selection, (6) data augmentation.
All thresholds are configurable via \texttt{configs/hyperparams.yaml}.

\subsection{Training Protocol}

\paragraph{MVAE Training.}
80 epochs with batch size 512, Adam optimiser ($\text{lr}=10^{-3}$, $\text{wd}=10^{-5}$), cosine LR annealing, gradient clipping at $\|\nabla\| \leq 1.0$.
KL warm-up over 10 epochs prevents posterior collapse.
Early stopping is \emph{not} used; all 80 epochs are completed.

\paragraph{CVAE Training.}
60 epochs with batch size 256, Adam ($\text{lr}=5\times10^{-4}$).
No warm-up is needed for the CVAE since it uses a standard Gaussian prior.

\paragraph{Model Selection.}
Best models are selected by minimum validation loss (on a held-out 10\% subset stratified by cluster).
Final metrics are reported on the full dataset.

\subsection{Software Dependencies}

Table~\ref{tab:software} lists all critical software dependencies and versions.
...
All source code, configuration files, trained models, and reproducibility scripts are publicly available at:

\begin{center}
\url{https://github.com/Runtime-Slayers/CITEDiscord-Net}
\end{center}

\noindent The repository includes:
\begin{itemize}[leftmargin=*]
  \item Complete source code for all 10 pipeline stages
  \item Hyperparameter configuration files (\texttt{configs/hyperparams.yaml})
  \item Pre-computed results, figures, and metrics
  \item Installation instructions and setup scripts
  \item This manuscript in \LaTeX{} source format
\end{itemize}

\noindent To reproduce all results:
\begin{verbatim}
git clone https://github.com/Runtime-Slayers/
    CITEDiscord-Net.git
cd CITEDiscord-Net
